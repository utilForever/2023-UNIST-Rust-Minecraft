...
use crate::constants::{
    FLYING_SPEED, FLYING_SPRINTING_SPEED, FLYING_TRIGGER_INTERVAL, FOV, HORIZONTAL_ACCELERATION,
    IN_AIR_FRICTION, JUMP_IMPULSE, MAX_VERTICAL_VELOCITY, MOUSE_SENSITIVITY_X, MOUSE_SENSITIVITY_Y,
    ON_GROUND_FRICTION, PLAYER_EYES_HEIGHT, PLAYER_HALF_WIDTH, PLAYER_HEIGHT, PLAYER_WIDTH,
    SNEAKING_SPEED, SPRINTING_SPEED, SPRINTING_TRIGGER_INTERVAL, WALKING_SPEED,
};
...
impl PlayerProperties {
    ...
    pub fn handle_input_event(&mut self, event: &glfw::WindowEvent) {
        match event {
            glfw::WindowEvent::Key(glfw::Key::Space, _, glfw::Action::Press, _) => {
                ...
                } else if Instant::now().duration_since(self.fly_last_toggled)
                    < *FLYING_TRIGGER_INTERVAL
                {
                    self.is_flying = !self.is_flying;
                    println!("Flying: {}", self.is_flying);
            ...
            glfw::WindowEvent::Key(glfw::Key::W, _, glfw::Action::Press, _) => {
                ...
                } else if Instant::now().duration_since(self.sprint_last_toggled)
    ...
    pub fn on_update(
        ...
        if self.is_sprinting && !input_cache.is_key_pressed(glfw::Key::W) {
            self.is_sprinting = false;
        }
...
impl PlayerPhysicsState {
    ...
    pub fn apply_friction(&mut self, dt: f32, is_vertically: bool) {
