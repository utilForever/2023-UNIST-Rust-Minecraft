...
impl ChunkManager {
    ...
    pub fn get_block(&self, x: i32, y: i32, z: i32) -> Option<BlockID> {
        ...
        match self.get_column(chunk_x, chunk_z) {
            Some(col) => Some(col.get_chunk(chunk_y).get_block(block_x, block_y, block_z)),
            None => None,
        }
    }
    ...
    pub fn set_block(&mut self, x: i32, y: i32, z: i32, block: BlockID) -> bool {
        ...
        match self.get_column(chunk_x, chunk_z) {
            Some(col) => {
                Some(
                    col.get_chunk(chunk_y)
                        .set_block(block_x, block_y, block_z, block),
                );
                true
            }
            None => false,
        ...
        for x in -1..=1 {
            for z in -1..=1 {
                neighbourhood[3 * (x + 1) as usize + (z + 1) as usize] = if x == 0 && z == 0 {
                    ...
                    self.loaded_chunk_columns
                        .get(&(chunk_x + x, chunk_z + z))
                        .map(|column| Arc::clone(column.value()))
                };
            }
        }
    ...
    pub fn update_block(
        ...
        let active_faces_of_block = {
            let column = self.get_column(chunk_x, chunk_z).unwrap();
