...
pub struct ChunkManager {
    ...
    _fresh_chunk: HashSet<(i32, i32, i32)>,
...
impl ChunkManager {
    ...
    pub fn get_chunk(&self, x: i32, y: i32, z: i32) -> Option<&Chunk> {
        ...
        self.loaded_chunk_columns
            .get(&(x, z))
            .map(|column| &column.chunks[y as usize])
    }
    ...
    pub fn get_chunk_mut(&mut self, x: i32, y: i32, z: i32) -> Option<&mut Chunk> {
        ...
        self.loaded_chunk_columns
            .get_mut(&(x, z))
            .map(|column| &mut column.chunks[y as usize])
    }
    ...
    pub fn single_chunk(&mut self) {
        self.add_chunk_column(
            (0, 0),
            Box::new(ChunkColumn::full_of_block(BlockID::Cobblestone)),
        );
    }
    ...
    pub fn rebuild_dirty_chunks(&mut self, uv_map: &TexturePack) {
        ...
        for (&(chunk_x, chunk_y, chunk_z), dirty_blocks) in changelist_per_chunk.iter() {
            ...
            self.upload_chunk_to_gpu(chunk_x, chunk_y, chunk_z, &uv_map);
        }
    }
