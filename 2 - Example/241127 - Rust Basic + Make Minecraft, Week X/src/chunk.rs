...
impl Chunk {
    ...
    pub fn reset(&mut self) {
        ...
        self.number_of_blocks = CHUNK_VOLUME;
        self.active_faces = BitVec::from_elem(6 * CHUNK_VOLUME as usize, false);
        self.ao_vertices = vec![[[0; 4]; 6]; CHUNK_VOLUME as usize];
    ...
    pub fn unload_from_gpu(&mut self) {
        gl_call!(gl::NamedBufferData(self.vbo, 0, null(), gl::DYNAMIC_DRAW));
    }

    pub fn upload_to_gpu(&mut self, texture_pack: &TexturePack) {
        let visible_faces_cnt = self.active_faces.iter().fold(0, |acc, b| acc + b as i32);
