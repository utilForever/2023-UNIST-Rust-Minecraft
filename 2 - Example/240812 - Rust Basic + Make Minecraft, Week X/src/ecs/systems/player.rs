...
use crate::constants::{
    FLYING_TRIGGER_INTERVAL, FOV, JUMP_IMPULSE, PLAYER_EYES_HEIGHT, REACH_DISTANCE,
...
impl<'a> System<'a> for HandlePlayerInput {
    ...
    fn run(&mut self, data: Self::SystemData) {
        ...
        for (player_state, player_physics_state, inventory) in
            (&mut player_state, &mut player_physics_state, &inventory).join()
        {
                    ...
                    if input_cache.is_mouse_button_pressed(glfw::MouseButtonLeft) {
                        ...
                        if let Some(((x, y, z), normal)) = &player_state.targeted_block {
                            place_block(
                                (*x, *y, *z),
                                &normal,
                                &player_physics_state.get_latest_state().aabb,
                                &inventory,
                                &mut chunk_manager,
                            );
                        }
