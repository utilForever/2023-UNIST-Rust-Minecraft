...
fn main() {
    ...
    while !window.should_close() {
                    ...
                    if let &Some(((x, y, z), normal)) = &looking_block {
                        match button {
                            MouseButton::Button1 => {
                                ...
                                block_particles.spawn_block_breaking_particles(
        ...
        player_interpolator.update_player_physics(
            global_timer.time(),
            &input_cache,
            &chunk_manager,
        ...
        if input_cache.is_key_pressed(Key::LeftShift) && player_physics_state.is_on_ground {
            player_properties.is_sneaking = true;
            player_properties.is_sprinting = false;
        }

        if input_cache.is_key_pressed(Key::LeftControl)
            && input_cache.is_key_pressed(Key::W)
            && !player_properties.is_sneaking
        {
            player_properties.is_sprinting = true;
        }

        if player_properties.is_sprinting && !input_cache.is_key_pressed(Key::W) {
            player_properties.is_sprinting = false;
        }
        ...
        player_properties
            .camera_height
            .interpolate_camera_height(global_timer.time(), target_camera_height);
